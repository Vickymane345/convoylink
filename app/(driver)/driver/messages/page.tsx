...
export default async function DriverMessagesPage() {
  ...
  const { data: bookingsRaw } = await (admin as any)
    .from('bookings')
    .select(`
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-zinc-400 mt-1">Chat with customers about your assignments</p>
      </div>
      <BookingInbox threads={threads} role="driver" />
    </div>
  )
}
