...
export default async function DriverThreadPage({
  ...
  const { data: booking } = await (admin as any)
    .from('bookings')
    .select('id, service_type, pickup_location, dropoff_location, status, driver_id')
    .eq('id', bookingId)
    .eq('driver_id', user.id)
    .single() as { data: { id: string; service_type: string; pickup_location: string; dropoff_location: string; status: string; driver_id: string } | null }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-theme(spacing.16))]">
      <div className="flex items-center gap-3 mb-4 shrink-0">
        <Link href="/driver/messages" className="text-zinc-400 hover:text-white transition-colors">
