...
export default async function CustomerThreadPage({
  ...
  const { data: booking } = await (supabase as any)
    .from('bookings')
    .select('id, service_type, pickup_location, dropoff_location, status, customer_id')
    .eq('id', bookingId)
    .eq('customer_id', user.id)
    .single() as { data: { id: string; service_type: string; pickup_location: string; dropoff_location: string; status: string; customer_id: string } | null }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-theme(spacing.16))]">
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        <Link href="/dashboard/messages" className="text-zinc-400 hover:text-white transition-colors">
